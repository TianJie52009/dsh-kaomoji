...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      "mode.auto": "智能",
      "mode.auto.desc": "适合表达友好、共情或俏皮情绪时自然使用",
      "mode.frequent": "高频",
      ...
      "status.unsupported": "设置仅在 Host 本机（loopback）页面可修改。",
    ...
    const en = {
      ...
      "mode.auto": "Smart",
      "mode.auto.desc": "Use naturally when a friendly, empathetic, or playful mood fits",
      ...
      "status.unsupported": "Settings can only be changed on the Host’s local (loopback) page.",
     ...
     */
    function createSettingsStore(rpc, isLoopback) {
      let snapshot = {
        status: isLoopback ? "loading" : "unavailable",
        settings: cloneSettings(DEFAULT_SETTINGS),
        revision: undefined,
        writable: Boolean(isLoopback),
        saveError: null,
        justSaved: false,
      };
      ...
      const callRpc = async (endpoint, payload) => {
        ...
        try {
          ...
          return result && result.ok === true ? result.value : undefined;
        } catch {
          return undefined;
        }
      };
      ...
      return {
        ...
        async refresh() {
          const value = await callRpc("get", {});
          if (value === undefined) {
            if (isLoopback) publish({ status: "ready", saveError: "load" });
            return;
          }
          publish({
            status: "ready",
        ...
        patch(field) {
          ...
          return enqueue(async () => {
            const value = await callRpc("save", {
              settings: next,
              expectedRevision: snapshot.revision,
            });
            if (value === undefined) {
              publish({ saveError: "save", justSaved: false });
              return;
            }
            publish({
              settings: normalizeSettings(value.settings),
        ...
        async reset() {
          ...
          const value = await callRpc("reset", { expectedRevision: snapshot.revision });
          if (value === undefined) {
            publish({ saveError: "save", justSaved: false });
            return;
          }
    ...
    function apply(ctx) {
      ...
      const controller = createSettingsStore(connection.rpc, connection.isLoopback);
