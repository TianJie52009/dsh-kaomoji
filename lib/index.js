...
export function apply(ctx, config = {}) {
  ...
  // 设置读写走 webServer 直连路由，不用 connection.rpc：后者在部分部署里
  // connection 服务不进入插件作用域，通道永远注册不上（浏览器侧表现为 405）。
  ...
  const attachSettingsRoute = (scope) => {
    scope.effect(
      ...
      "dsh-kaomoji: settings route",
    );
    console.log(`[dsh-kaomoji] settings route 已注册（path=${SETTINGS_RPC_CHANNEL}）`);
  };
